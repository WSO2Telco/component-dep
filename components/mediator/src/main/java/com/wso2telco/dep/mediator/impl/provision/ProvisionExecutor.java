...
public class ProvisionExecutor extends RequestExecutor {

	private Log log = LogFactory.getLog(ProvisionExecutor.class);
	...
	public boolean execute(MessageContext context) throws CustomException, AxisFault, Exception {
		...
		try {
			...
			ProvisionHandler handler = getProvisionHandler(getSubResourcePath());
			return handler.handle(context);
		} catch (JSONException e) {

			log.error("faild to return provision handler in execute : ", e);
	...
	public boolean validateRequest(String httpMethod, String requestPath, JSONObject jsonBody, MessageContext context)
		...
		ProvisionHandler handler = getProvisionHandler(requestPath);
		return handler.validate(httpMethod, requestPath, jsonBody, context);
	}

	private ProvisionHandler getProvisionHandler(String requestPath) {

		if (handler == null) {

			handler = ProvisionHandlerFactory.createProvisionHandler(requestPath, this);
		}
