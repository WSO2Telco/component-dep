 ...
 ******************************************************************************/
package com.wso2telco.mediator.impl.payment;
...
import com.wso2telco.mediator.ResponseHandler;
import com.wso2telco.mediator.internal.AggregatorValidator;
...
//        return false;
    ...
    public boolean validateRequest(String httpMethod, String requestPath, JSONObject jsonBody, MessageContext
        ...
        // 100 - Charge, 101 - Refund, 102 - Reserve amount, 103 - Reserve additional amount, 104 - Charge against reservation, 105 - Release Reservation, 
        // 106 - List transactions, 0 - Unsupported
        
        context.setProperty("mife.prop.operationType", apiType);
        if(apiType == 100){
        ...
        JSONObject chargingdmeta = jsonBody.getJSONObject("amountTransaction").getJSONObject("paymentAmount").getJSONObject("chargingMetaData");
        ...
        context.setProperty("mife.prop.merchantId", merchantId);
        context.setProperty("mife.prop.category", categoryCode);
        context.setProperty("mife.prop.subCategory", subCategory);
        ...
        if ( (categoryCode.length() == 12) && (categoryCode.matches("[-+]?\\d*\\.?\\d+")) ) {    
            if(categoryCode.substring(0, 3).equals("000")){
                ...
                context.setProperty("mife.prop.merchantId", categoryCode.substring(0, 3));
            }
            
            if(categoryCode.substring(3, 6).equals("000")){
                context.setProperty("mife.prop.category", "");
            } else {
                context.setProperty("mife.prop.category", categoryCode.substring(3, 6));
            }
            
            if(categoryCode.substring(6, 9).equals("000")){
                context.setProperty("mife.prop.subCategory", "");
            } else {
                context.setProperty("mife.prop.subCategory", categoryCode.substring(6, 9));
            }
        }
...
//                routeToEndpoint(endpoint, mc, sending_add);
        ...
        if (isaggrigator) {
            ...
            if (!chargingdmeta.isNull("onBehalfOf")) {
                ...
                mc.setProperty("mife.prop.merchantId", chargingdmeta.getString("onBehalfOf"));
            }
        }
        ...
        if ((!chargingdmeta.isNull("purchaseCategoryCode")) && (!chargingdmeta.getString("purchaseCategoryCode")
            ...
            mc.setProperty("mife.prop.category",chargingdmeta.getString("purchaseCategoryCode"));
        }
     ...
     *
     * @param chargingdmeta the chargingdmeta
     * @param lstCategories the lst categories
