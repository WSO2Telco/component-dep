...
import com.wso2telco.dep.ratecardservice.service.APIOperationService;
...
public class OperationResource {
	...
	private final Log log = LogFactory.getLog(OperationResource.class);
	...
	public Response getAPIOperations(@PathParam("apiName") String apiName, @QueryParam("schema") String schema) {
		...
		log.debug("OperationResource getAPIOperations -> response code : " + responseCode);
		log.debug("OperationResource getAPIOperations -> response body : " + responseString);
	...
	public Response getOperationRates(@PathParam("apiName") String apiName, @QueryParam("schema") String schema) {
		...
		try {
			...
			if (rate != null) {
				...
				log.error(
						"Error in OperationResource getOperationRates : operation rates are not found in database ");
				throw new BusinessException(ServiceError.NO_RESOURCES);
			}
		} catch (BusinessException e) {
		...
		log.debug("OperationResource getOperationRates -> response code : " + responseCode);
		log.debug("OperationResource getOperationRates -> response body : " + responseString);
	...
	public Response getAPIOperationRates(@PathParam("apiName") String apiName, @PathParam("apiOperationId") int apiOperationId, @QueryParam("schema") String schema) {
		...
		try {
			...
			if (!rateDefinitions.isEmpty()) {
				...
				log.error("Error in OperationRateResource getAPIOperationRates : api operation rates are not found in database ");
				throw new BusinessException(ServiceError.NO_RESOURCES);
			}
		} catch (BusinessException e) {
		...
		log.debug("OperationRateResource getAPIOperationRates -> response code : " + responseCode);
		log.debug("OperationRateResource getAPIOperationRates -> response body : " + responseString);

		return Response.status(responseCode).entity(responseString).build();
	}

	@Path("/{apiOperationId}/operationrates")
