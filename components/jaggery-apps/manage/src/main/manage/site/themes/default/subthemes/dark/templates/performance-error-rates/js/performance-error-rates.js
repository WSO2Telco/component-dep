...
function getSelectedSubscriber() {
	...
	if (subscriber.trim() != "__ALL__") {
		...
		jagg.post("/site/blocks/performance-error-rates/ajax/performance-error-rates.jag", {
			action : action,
...
function getErrorResponseCodes() {
	$("#error-details").css('display', 'none');
	...
	var action = "getErrorResponseCodes";
	jagg
			.post(
					"/site/blocks/performance-error-rates/ajax/performance-error-rates.jag",
					{
						action : action,
						fromDate : fromDate,
						toDate : toDate,
					...
					function(result) {
						if (!result.error) {
							if (result.pieChart != null) {		
									var error_response_codes_for_pieChart = result.pieChart;
									if (error_response_codes_for_pieChart.length > 0) {
										$('#error-response-codes-pie-chart').empty();
										generateErrorResponseCodesPicChart(error_response_codes_for_pieChart);
										$("#error-details").css('display', 'inline');
									} else {
										$('#error-response-codes-pie-chart').empty();
										$("#error-response-codes-pie-chart").text("There is no API Usage");
									}
									
									var error_response_codes_for_histogram = result.histogram;
									if (error_response_codes_for_histogram.length > 0) {
										$('#error-response-codes-histogram').empty();
										generateErrorResponseCodesHistogram(error_response_codes_for_histogram);
									} else {
										$('#error-response-codes-histogram').empty();
										$('#legend').empty();
										$("#error-response-codes-histogram").text(
...
function generateErrorResponseCodesPicChart(dataMap) {
	var errorResponseCodesPiechart = "error-response-codes-pie-chart";
	...
	"dojo/domReady!"
		...
		dojo.empty(errorResponseCodesPiechart);

		// Create the chart within it's "holding" node
		var apiUsageChart = new Chart(errorResponseCodesPiechart);
...
function generateErrorResponseCodesHistogram(error_response_codes_for_histogram) {

	var errorResponseCodesHistogram = "error-response-codes-histogram";
	require(
			[
			// Require the basic chart class
			"dojox/charting/Chart",
			...
			function(Chart, theme, LinesPlot, Legend, Tooltip, Magnify) {
				...
				// Create the chart within it's "holding" node
				var chart = new Chart(errorResponseCodesHistogram);
				...
				var maxArray = [];
				for (var i = 0; i < error_response_codes_for_histogram.length; i++) {
					maxArray[i] = findMax(error_response_codes_for_histogram[i].apiHits);
				}
				...
				// Add axes
				var xAxisLabels = createXAxisLabels(error_response_codes_for_histogram[0].apiHitDates.reverse());
				chart.addAxis("x", {
					title:"date",
				...
				for (var i = 0; i < error_response_codes_for_histogram.length; i++) {
					var apiHitsArray = [];
					for (var j = 0; j < error_response_codes_for_histogram[i].apiHits.length; j++) {
						apiHitsArray[j] = parseInt(error_response_codes_for_histogram[i].apiHits[j]);
					}
					
					var histogramLineName = error_response_codes_for_histogram[i].errorCode;
					
					chart.addSeries(histogramLineName, apiHitsArray);
				}
