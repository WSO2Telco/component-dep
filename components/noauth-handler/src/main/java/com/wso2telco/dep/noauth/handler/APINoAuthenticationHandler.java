...
public class APINoAuthenticationHandler extends APIAuthenticationHandler {
	...
	private void handleIPValidateFailure(MessageContext messageContext, APISecurityException e) {
		...
		// This property need to be set to avoid sending the content in pass-through
		...
		try {
			...
			// In case of an error it is logged and the process is continued because we're
			// setting a fault message in the payload.
