...
public class BillingDAO {
	...
	public void printAPIRequestSummary() throws Exception {
		...
		sql.append("select * from ").append(
				ReportingTable.API_REQUEST_SUMMARY.getTObject());
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				System.out.println("=== Results for api :"
						+ rs.getString("api") + " , ck :  "
	...
	public void printSouthboundTraffic() throws Exception {
		...
		try {
			...
			while (rs.next()) {
				log.debug("=== Results for southbound Traffic :"
						+ rs.getString("ENDPOINT") + " , endpoint :  "
	 ...
	 *             the exception
	 */
	public List<APIResponseDTO> getAllResponseTimesForAPI(String operator,
			String appId, String apiVersion, String fromDate, String toDate)
			throws Exception {
		...
		try {
			connection = DbUtils
					.getDbConnection(DataSourceNames.WSO2AM_STATS_DB);
			ps = connection.prepareStatement(sql.toString());
			log.debug("getAllResponseTimesForAPI for apiVersion---> "
					+ apiVersion);
			ps.setString(1, apiVersion);
	 ...
	 *             the exception
	 */
	public Map<String, Integer> getAPICountsForApplication(String consumerKey,
			String year, String month, String userId) throws Exception {
		Connection connection = null;
		PreparedStatement ps = null;
		ResultSet results = null;
		...
		try {
			...
			results = ps.executeQuery();
			while (results.next()) {
				apiCount.put(results.getString("api_version"),
	 ...
	 *             the exception
	 */
	public List<OperatorDetailsEntity> getOperatorDetailsOfSubscription(
			int applicationId, int apiId) throws Exception {
		Connection connection = null;
		PreparedStatement ps = null;
		ResultSet results = null;
		StringBuilder sql = new StringBuilder();
		...
		try {
			...
			;
			ps = connection.prepareStatement(sql.toString());
			log.debug("getOperatorDetailsOfSubscription for applicationId---> "
			...
			while (results.next()) {
				...
				detailsEntity.setOperatorName(results
	 ...
	 *             the exception
	 */
	public List<OperatorDetailsEntity> getDetailsOfSubscription(
			int applicationId, int apiId) throws Exception {
		Connection connection = null;
		PreparedStatement ps = null;
		ResultSet results = null;
		...
		try {
			...
			while (results.next()) {
				...
				detailsEntity.setOperationId(Integer.parseInt(results
	 ...
	 *             the exception
	 */
	public Map<CategoryCharge, BilledCharge> getAPICountsForSubscription(
			String consumerKey, short year, short month, String apiName,
			String apiVersion, String operatorId, int operationId)
			throws Exception {
		Connection connection = null;
		PreparedStatement ps = null;
		ResultSet results = null;
		StringBuilder sql = new StringBuilder();
		sql.append(
				"SELECT category,subcategory,sum(response_count) as total, sum(chargeAmount) as totalPayAmount ")
				.append("from ")
		...
		try {
			...
			while (results.next()) {
				categoryCharge = new CategoryCharge(operationId,
						results.getString("category"),
						results.getString("subcategory"));
				apiCount.put(categoryCharge,
						new BilledCharge(results.getInt("total")));
			}
		} catch (Exception e) {
		...
		log.debug("apiCount :" + apiName + " :" + apiVersion + ": "
				+ consumerKey);
		return apiCount;
	}
	 ...
	 *             the exception
	 */
	public Map<CategoryCharge, BilledCharge> getNorthboundAPICountsForSubscription(
			String consumerKey, short year, short month, String apiName,
			String apiVersion, int operationId) throws Exception {
		Connection connection = null;
		PreparedStatement ps = null;
		ResultSet results = null;
		StringBuilder sql = new StringBuilder();
	 ...
	 *             the exception
	 */
	public Map<String, Integer> getAPICountsForApplicationOpco(
			String consumerKey, String year, String month, String userId,
		...
		try {
			...
			results = ps.executeQuery();
			while (results.next()) {
				apiCount.put(results.getString("operatorId"),
						results.getInt("total"));
			}
		} catch (Exception e) {
	...
	public void printAPISubscriberTable() throws Exception {
		...
		sql.append("select * from ").append(
				ReportingTable.AM_SUBSCRIBER.getTObject());
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				System.out.println("=== Results for SUBSCRIBER_ID  :"
						+ rs.getInt("SUBSCRIBER_ID") + " , USER_ID :  "
						+ rs.getString("USER_ID") + " , DATE_SUBSCRIBED : "
	...
	public List<String> getAllSubscriptions() throws Exception {
		StringBuilder sql = new StringBuilder();
		sql.append("select USER_ID from ").append(
				ReportingTable.AM_SUBSCRIBER.getTObject());
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public int getNoOfSubscribers(String subscriber, String app, String apiName)
			throws Exception {
	...
	public Set<PaymentRequestDTO> getPaymentAmounts(short year, short month,
			String consumerKey, String api_version, String operatorId)
			throws Exception {
		...
		sql.append(
				"SELECT api,userId,consumerKey,chargeAmount,purchaseCategoryCode,time FROM ")
				.append(HostObjectConstants.SB_RESPONSE_SUMMARY_TABLE)
				.append(" WHERE api_version=? and consumerKey=? and operatorId =? and responseCode like '2%' and  month=? and year=? ");
		...
		try {
			...
			while (results.next()) {
				...
				paymentRequest.setDate(new SimpleDateFormat(
						"yyyy-MM-dd HH:mm:ss").parse(rqdate));
		...
		log.debug("done getPaymentAmounts :" + consumerKey + " :" + api_version
				+ " :" + operatorId);
		return requestSet;
	}
	 ...
	 */
	// Retriving amount charged from end-user through payment API
	public Set<PaymentRequestDTO> getNbPaymentAmounts(short year, short month,
			String consumerKey, String api_version, int operation,
			String category, String subcategory) throws Exception {
		...
		sql.append(
				"SELECT api,userId,consumerKey,chargeAmount,category,subcategory,merchantId,time FROM ")
				.append(HostObjectConstants.NB_RESPONSE_SUMMARY_TABLE)
	 ...
	 *             the exception
	 */
	public int getSubscriptionIdForApplicationAPI(int appId,
			APIIdentifier apiIdent) throws Exception {
		StringBuilder sql = new StringBuilder();
		...
		try {
			...
			if (subscriptionId == -1) {
				String msg = "Unable to find the subscription ID for API: "
						+ apiIdent + " in the database";
				log.error(msg);
	 ...
	 *             the exception
	 */
	public List<String[]> getTotalAPITrafficForPieChart(String fromDate,
		...
		if (applicationId == 0) {
			...
			consumerKey = apiManagerDAO
					.getConsumerKeyByApplication(applicationId);
		}
		...
		try {
			...
			while (results.next()) {
				String[] temp = { results.getString("api"),
						results.getString("api_request_count") };
	 ...
	 *             the exception
	 */
	public List<String[]> getTotalAPITrafficForHistogram(String fromDate,
		...
		List<String[]> api_list = apiManagerDAO
				.getAPIListForAPITrafficHistogram(fromDate, toDate, api);
		...
		sql.append(
				"select api,date(time) as date, sum(response_count) hits from ")
				.append(ReportingTable.SB_API_RESPONSE_SUMMARY.getTObject())
		...
		try {
			...
			while (results.next()) {
				String[] temp = { results.getString(1),
						results.getDate(2).toString(), results.getString(3) };
				api_request.add(temp);
			}

			for (int i = 0; i < api_request.size(); i++) {
				String apiRequestNameNDate = api_request.get(i)[0].toString()
						+ "_" + api_request.get(i)[1].toString();
				for (int j = 0; j < api_list.size(); j++) {
					String apiNameNDate = api_list.get(j)[0].toString() + "_"
							+ api_list.get(j)[1].toString();
	 ...
	 *             the exception
	 */
	public List<String[]> getOperatorWiseAPITrafficForPieChart(String fromDate,
		...
		sql.append(
				"SELECT operatorId, SUM(response_count) AS api_request_count ")
				.append("FROM ")
				.append(ReportingTable.SB_API_RESPONSE_SUMMARY.getTObject())
		...
		try {
			...
			results = ps.executeQuery();
			while (results.next()) {
				String[] temp = { results.getString("operatorId"),
						results.getString("api_request_count") };
				api_request.add(temp);
			}
		} catch (Exception e) {
	 ...
	 *             the exception
	 */
	public List<String[]> getApprovalHistory(String fromDate, String toDate,
			String subscriber, String api, int applicationId, String operator)
			throws Exception {
		...
		sql.append(
				"select application_id,name,if(description is null,'Not Specified',description) as description,")
		...
		try {
			...
			while (rs.next()) {
				...
				if (!rs.getString("name")
						.equalsIgnoreCase("DefaultApplication")) {
					// get operator list
					String operators = OperatorDAO
							.getApprovedOperatorsByApplication(
									rs.getInt("application_id"), operator);
					if (operator.equals("__ALL__")) {
						String[] temp = {
								String.valueOf(rs.getInt("application_id")),
								rs.getString("name"),
								rs.getString("description"),
								rs.getString("app_status"), operators };
						applist.add(temp);
					} else {
	 ...
	 *             the exception
	 */
	public List<Approval> getApprovalHistoryApp(int applicationId,
		...
		sql.append(
				"SELECT application_id, sub_status,tier_id, api_name,api_version FROM ")
				.append(ReportingTable.AM_SUBSCRIPTION.getTObject())
				.append(", ").append(ReportingTable.AM_API.getTObject())
				.append(" WHERE ")
				.append(ReportingTable.AM_SUBSCRIPTION.getTObject())
				.append(".api_id = ")
				.append(ReportingTable.AM_API.getTObject()).append(".api_id ")
		...
		try {
			...
			applist.add(new Approval(String.valueOf(applicationId), "1",
					application.getName(), 0, appstatus, "", "", "", null, null));
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				Approval temp = new Approval(rs.getString("application_id"),
						"3", rs.getString("api_name"), 0,
						rs.getString("sub_status"), rs.getString("tier_id"),
	...
	public List<String[]> getAllOperationTypes() throws Exception {
		...
		sql.append("SELECT `operation_id`, `operation` FROM ").append(
				ReportingTable.API_OPERATION_TYPES.getTObject());
	 ...
	 *             the exception
	 */
	public List<String[]> getAllAPIs(String fromDate, String toDate,
	 ...
	 *             the exception
	 */
	public List<String[]> getAllErrorResponseCodes(String fromDate,
			String toDate, String subscriber, String operator,
			int applicationId, String api) throws Exception {
		String consumerKey = null;
		if (subscriber.equals("__ALL__")) {
		...
		sql.append(
				"SELECT DISTINCT IFNULL(exceptionId, 'SVC1000') AS exceptionId FROM ")
				.append(ReportingTable.SB_API_RESPONSE_SUMMARY.getTObject())
				.append(" WHERE userId LIKE ? AND consumerKey LIKE ? AND operatorId LIKE ? AND api LIKE ? AND (STR_TO_DATE(")
				.append(ReportingTable.SB_API_RESPONSE_SUMMARY.getTObject())
	 ...
	 *             the exception
	 */
	public List<String[]> getCustomerCareReportData(String fromDate, String toDate, String msisdn, String subscriber, String operator, String app, String api, String startLimit, String endLimit) throws Exception {
        ...
        try {
            ...
            results = ps.executeQuery();
            while (results.next()) {
            	String[] temp = {results.getString(1), results.getString(2), results.getString(3)};
	 ...
	 *             the exception
	 */
	public List<String[]> getAPIWiseTrafficForReport(String fromDate,
		...
		sql.append(
				"SELECT time, userId, operatorId, requestId, msisdn, response_count, responseCode, jsonBody, resourcePath, method,api FROM ")
				.append(ReportingTable.SB_API_RESPONSE_SUMMARY.getTObject())
				.append(" WHERE ")
				.append(responseStr)
				...
				if (!jsonBody.isEmpty()) {
					...
					try {
						...
						if (!homejson.isNull("outboundSMSMessageRequest")) {
							...
							if (!smsTransactionObject
									.isNull("clientCorrelator")) {
								clientCorelator = smsTransactionObject
										.getString("clientCorrelator");
							}
						}
					} catch (Exception ex) {
						log.debug("Unable to read JSON body stored in DB :: "
								+ ex);
						clientCorelator = "";
					}
				...
				if (!requestUrl.isEmpty() && !requestapi.isEmpty()) {
					...
					if (apitype.equalsIgnoreCase("send_sms")) {
						event_type = "Outbound";
							...
							.equalsIgnoreCase("retrive_sms_subscriptions")) {
						...
						event_type = "USSD Subscription";
							...
							.equalsIgnoreCase("stop_ussd_subscription")) {
						if (requestMethod.equalsIgnoreCase("DELETE")) {
	 ...
	 *             the exception
	 */
	public ResultSet getTxLogData(String fromDate, String toDate,
			String subscriber, String operator, int opType, String dataType)
			throws Exception {
		if (subscriber.equals("__ALL__")) {
			subscriber = "%";
		}
	 ...
	 *             the exception
	 */
	public ResultSet getTxLogDataNB(String fromDate, String toDate,
			String subscriber, String operator, int opType, String dataType)
			throws Exception {
		if (subscriber.equals("__ALL__")) {
			subscriber = "%";
	 ...
	 *             the exception
	 */
	public List<String[]> getAPIWiseTrafficForReportCharging(String fromDate,
			String toDate, String subscriber, String operator, String api,boolean isError)
			throws Exception {
		if (subscriber.equals("__ALL__")) {
			subscriber = "%";
		...
		sql.append(
				"SELECT time, userId, operatorId, requestId, msisdn, chargeAmount, responseCode, jsonBody, resourcePath, method, purchaseCategoryCode,api FROM ")
				.append(ReportingTable.SB_API_RESPONSE_SUMMARY.getTObject())
				.append(" WHERE ")
				.append(responseStr)
				.append(" AND operatorId LIKE ? AND replace(userid,'@carbon.super','') LIKE ? AND api LIKE ? AND (day between ? and ? ) ");
			...
			while (results.next()) {
				...
				if (!jsonBody.isEmpty()) {
					try {
						...
						System.out
								.println("Unable to read JSON body stored in DB :: "
										+ ex);
						clientCorelator = "";
					}
				...
				String[] temp = { dateTime, results.getString(2),
						results.getString(3), event_type, results.getString(4),
						clientCorelator, results.getString(5),
						results.getString(6), currency, results.getString(7),
	 ...
	 *             the exception
	 */
	public List<String[]> getErrorResponseCodesForPieChart(String fromDate,
			String toDate, String subscriber, String operator,
			int applicationId, String api) throws Exception {
		String consumerKey = null;
		if (subscriber.equals("__ALL__")) {
		...
		sql.append(
				"SELECT IFNULL(exceptionId,'SVC1000') AS exceptionId, SUM(response_count) AS api_response_count FROM ")
				.append(ReportingTable.SB_API_RESPONSE_SUMMARY.getTObject())
				.append(" WHERE userId LIKE ? AND consumerKey LIKE ? AND operatorId LIKE ? AND (STR_TO_DATE(")
				.append(ReportingTable.SB_API_RESPONSE_SUMMARY.getTObject())
		...
		try {
			...
			while (results.next()) {
				String[] temp = { results.getString("exceptionId"),
						results.getString("api_response_count") };
	 ...
	 *             the exception
	 */
	public List<String[]> getErrorResponseCodesForHistogram(String fromDate,
			String toDate, String subscriber, String operator,
			int applicationId, String api) throws Exception {
		String consumerKey = null;
		if (subscriber.equals("__ALL__")) {
	...
	private String findAPIType(String ResourceURL, String requested_api) {
		...
		if (ResourceURL.toLowerCase().contains(outboundkeyString.toLowerCase())
			...
			apiType = "query_sms";
				...
				retriveSMSString.toLowerCase())
				&& ResourceURL.toLowerCase().contains(
						regKeyString.toLowerCase())) {
			apiType = "retrive_sms";
		} else if (!requested_api.isEmpty()
				&& !requested_api.toLowerCase().contains(
						ussdKeyString.toLowerCase())
				&& ResourceURL.toLowerCase().contains(
						retriveSMSString.toLowerCase())
				&& ResourceURL.toLowerCase().contains(
						subscriptionKeyString.toLowerCase())) {
			apiType = "retrive_sms_subscriptions";
				...
				paymentKeyString.toLowerCase())) {
			apiType = "payment";
		} else if (ResourceURL.toLowerCase().contains(
				delivaryNotifyString.toLowerCase())) {
	...
	public ArrayList<SPObject> generateSPList() throws Exception {
		...
		sql.append(
				"select distinct(application_id),name,authz_user from (select am.application_id,am.name,ac.authz_user ")
	...
	public SPObject generateSPObject(String appId) throws Exception {
		...
		sql.append(
				"select am.application_id,am.name,ac.authz_user,ac.ACCESS_TOKEN,ca.consumer_key,ca.consumer_secret ")
				.append("from IDN_OAUTH2_ACCESS_TOKEN ac,IDN_OAUTH_CONSUMER_APPS ca,")
				.append(ReportingTable.AM_APPLICATION.getTObject())
				.append(" am,")
				.append(ReportingTable.AM_APPLICATION_KEY_MAPPING.getTObject())
	 ...
	 *             the exception
	 */
	public List<String[]> getTotalAPITrafficForLineChart(String fromDate,
			String toDate, String subscriber, String operator,
			int applicationId, String api) throws Exception {
		String consumerKey = null;
		if (subscriber.equals("__ALL__")) {
		...
		try {
			...
			results = ps.executeQuery();
			while (results.next()) {
				String[] temp = { results.getDate(1).toString(),
						results.getString(2) };
				api_request.add(temp);
			}
	 ...
	 *             the exception
	 */
	public List<APIResponseDTO> getAllResponseTimesForAllAPIs(String operator,
			String userId, String fromDate, String toDate, String timeRange)
			throws Exception {
		...
		sql.append(
				"SELECT SUM(response_count) as sumCount, SUM(serviceTime) as sumServiceTime, STR_TO_DATE(time,'%Y-%m-%d') as date FROM ")
	 ...
	 *             the exception
	 */
	public List<APIResponseDTO> getAllResponseTimesForAPIbyDate(
			String operator, String userId, String fromDate, String toDate,
			String api) throws Exception {
		...
		sql.append(
				"SELECT api,SUM(response_count) as sumCount, SUM(serviceTime) as sumServiceTime, STR_TO_DATE(time,'%Y-%m-%d') as date FROM ")
				.append(ReportingTable.SB_API_RESPONSE_SUMMARY.getTObject())
	 ...
	 *             the exception
	 */
	public String[] getTimeConsumptionByAPI(String operator, String userId,
			String fromDate, String toDate, String api) throws Exception {
		...
		try {
			...
			while (results.next()) {
				...
				timeConsumerData[1] = Double.toString(results
						.getDouble("highestConsumption"));
				timeConsumerData[2] = Double.toString(results
						.getDouble("avgTotalConsump"));
	...
	public Date getSubscriptionCreatedTime(int appId, APIIdentifier apiIdent)
		...
		sql.append("SELECT WF.WF_CREATED_TIME FROM ")
				...
				.append(" WF ").append("WHERE ")
				.append("SUBS.APPLICATION_ID = ? ")
				.append("AND SUBS.API_ID = ? ").append("AND WF.WF_TYPE= ? ")
		...
		try {
			...
			if (log.isDebugEnabled()) {
				log.debug("Subscription creation Time of workflow for app "
						+ appId + " and API " + apiId + " - " + wfCreatedTime);
			}
		} catch (Exception e) {
		...
		return (wfCreatedTime != null) ? new Date(wfCreatedTime.getTime())
				: null;
	}
	 ...
	 *             the exception
	 */
	public Map<String, CommissionPercentagesDTO> getCommissionPercentages(
			String spId, Integer appId) throws Exception {
		Connection connection = null;
		PreparedStatement ps = null;
		ResultSet results = null;
		StringBuilder sql = new StringBuilder();
		...
		sql.append(
				"select id,subscriber,merchant_code,app_id,sp_commission,ads_commission,opco_commission from ")
		...
		try {
			...
			while (results.next()) {
				...
				dto.setSpCommission(new BigDecimal(results
						.getDouble("sp_commission")));
				dto.setAdsCommission(new BigDecimal(results
						.getDouble("ads_commission")));
				dto.setOpcoCommission(new BigDecimal(results
						.getDouble("opco_commission")));
	...
	public String convertToLocalTime(String timeOffset, String time) {
		...
		log.debug("system date time after adding systemtimezone === "
				+ systemDateTime.toString());

		int hours = -1 * offsetValue / 60;
		int minutes = offsetValue % 60;
		minutes = Math.abs(minutes);

		DateTimeZone localTimeZone = DateTimeZone.forOffsetHoursMinutes(hours,
				minutes);
