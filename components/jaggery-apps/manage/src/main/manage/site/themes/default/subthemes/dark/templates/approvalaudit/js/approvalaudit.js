...
function generateDataTable(object, currentPage,pageCount){
	...
	$.each(object, function(key, value){
		...
		loadPaginatorView(pageCount, currentPage);
...
function loadPaginatorView(numberOfPages, currentPage) {
	...
	var previousAppender = '<li><a href="javascript:getApprovalHistory(0)"><<</a></li>';
	if (currentPage == 0) {
	...
	for (var i = 0; i < numberOfPages; i++) {
		...
		if (i == currentPage) {
			...
			currentRow = '<li><a href="javascript:getApprovalHistory(' + (i * 10) + ')">'
				+ rowSticker + '</a></li>';
		}
		$(".pagination ul").append(currentRow);
	}
	var lastPage = numberOfPages - 1;// alert(lastPage);
	var postAppender = '<li><a href="javascript:getApprovalHistory(' + (lastPage * 10)
		+ ')">>></a></li>';
...
function getApprovalHistory(offset){
	...
	jagg.post("/site/blocks/approvalaudit/ajax/approvalaudit.jag", {
		...
		if (!result.error) {
			if (result.applist != null) {
				...
				if((offset == 0 && api_history.length > 1)){
					totalPageCount = Math.ceil(api_history[api_history.length-1] / 10);
					generateDataTable(api_history.slice(0, -1), offset / 10, totalPageCount);
				} else if ((offset > 0 && api_history.length > 0 )){
					generateDataTable(api_history, offset /10, totalPageCount);
				} else{
					$('#application_history_tbl_div').empty();
