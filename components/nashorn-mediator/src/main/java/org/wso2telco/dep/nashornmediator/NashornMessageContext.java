...
public class NashornMessageContext {
     ...
     *
     * @param payloadJson JSON payload passed as a Nashorn JSON object
     * @throws ScriptException Throws exception when internal error happens when setting the payload to axis2 message
     *                         context
     */
    public void setPayloadJSON(Object payloadJson) throws ScriptException {
        try {
            ScriptObjectMirror json = (ScriptObjectMirror) scriptEngine.eval("JSON");
            String jsonString = (String)json.callMember("stringify", payloadJson);
            ...
            messageContext.setProperty(JSON_OBJECT, payloadJson);
