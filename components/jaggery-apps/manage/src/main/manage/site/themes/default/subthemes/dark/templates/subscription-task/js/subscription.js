...
function setChargeRates(taskId, rateName) {
	...
	jagg.post("/site/blocks/subscription-task/ajax/subscription-task.jag", { action:"setChargeRates",appId:appId,apiId:apiId,opName:operatorName,rateName:rateName,apiName:apiName,taskType:"subscription" },
			function (json) {
             if (!json.error) {
             } else {
                 jagg.showLogin();
...
function setratecarddropdown(result , taskID , IsHubRates) {
	...
	if(result.apiRates.length != 0){
		...
		$('#rateCardContainer').css("display", "block");
	}
